...
const FALLBACK_HEADERS = ["الاسم", "الدولة", "الاتحاد", "البريد الإلكتروني", "رقم الهاتف"];
...
const NORMALIZED_COUNTRIES = new Set([...ARAB_COUNTRIES].map((c) => normalizeAr(c)));
export { ARAB_COUNTRIES };

export async function parseFile(file: File): Promise<ParsedTable> {
  const name = file.name.toLowerCase();
  if (name.endsWith(".xlsx") || name.endsWith(".xls") || name.endsWith(".ods")) {
    return await parseSpreadsheetBuffer(await file.arrayBuffer());
  }
  if (name.endsWith(".csv") || name.endsWith(".tsv") || name.endsWith(".csx") || name.endsWith(".txt")) {
    return parseTextOrCsv(await decodeTextFile(file));
  }
  if (name.endsWith(".docx")) {
    return await parseDocxBuffer(await file.arrayBuffer());
  }
  if (name.endsWith(".doc")) {
    try {
      return await parseDocxBuffer(await file.arrayBuffer());
    } catch {
      return parseTextOrCsv(await file.text());
    }
  }
  return parseTextOrCsv(await file.text());
}
...
export async function decodeTextFile(file: File): Promise<string> {
  ...
  try { buffer = await file.arrayBuffer(); } catch { return file.text(); }
...
function needsFallbackDecoding(text: string): boolean {
  ...
  return ((text.match(/[\u00C0-\u00FF]/g) || []).length) > 4;
}

export async function parseSpreadsheetBuffer(buffer: ArrayBuffer | ArrayBufferView): Promise<ParsedTable> {
  ...
  if (!firstSheetName) throw new Error("لم يتم العثور على أي أوراق عمل داخل ملف الإكسيل.");
    ...
    if (tableMatches.length > 0) {
      ...
      for (const tbl of tableMatches) {
        ...
        for (const tr of trMatches) {
          ...
          if (rowCells.some((c) => c !== "")) tableRows.push(rowCells);
        }
      }
      if (tableRows.length > 0) return analyzeTableRows(tableRows);
    }
  } catch (err) {
...
export function parseTextOrCsv(rawText: string): ParsedTable {
  ...
  if (delimiter) {
    return analyzeTableRows(rawLines.map((line) => parseCsvLine(line, delimiter as string)));
  }
...
function parseTextListItem(line: string): string[] | null {
  ...
  if (emailMatch) { email = emailMatch[0]; rest = rest.replace(email, " ").trim(); }
  let phone = "";
  const phoneMatch = rest.match(/(\+?\d[\d\s-]{7,}\d)/);
  if (phoneMatch) { phone = phoneMatch[0].trim(); rest = rest.replace(phone, " ").trim(); }
  let parts: string[] = [];
  ...
  return [parts[0]?.trim() || "", parts[1]?.trim() || "", parts[2]?.trim() || parts[1]?.trim() || "", email, phone];
}
...
export function analyzeTableRows(rawRows: string[][]): ParsedTable {
  ...
  for (let i = 0; i < maxScan; i++) {
    ...
    if (score > highestScore) { highestScore = score; bestHeaderIdx = i; }
  }
  let headers: string[] = [];
  let dataRows: string[][] = [];
...
export function finalizeTable(headers: string[], rows: string[][]): ParsedTable {
  ...
  return { headers: normalizedHeaders, rows, suggestedMapping: mapping, analysis, warnings };
}
...
function isLikelyFederation(value: string): boolean {
  ...
  return v.includes("اتحاد") || v.includes("نادي") || v.includes("فدرالي") || v.includes("هيئه") || v.includes("لجنه") || v.includes("federation") || v.includes("club") || v.includes("association") || v.includes("union");
}

function detectSequence(values: string[]): { sequential: boolean; startOffset: number } {
  const nums = values.map((v) => Number(String(v).replace(/[^\d-]/g, "")));
  if (nums.length < 2 || nums.some((n) => !Number.isFinite(n))) return { sequential: false, startOffset: 0 };
  for (const start of [nums[0], nums[0] - 1]) {
    let matched = 0;
    for (let i = 0; i < nums.length; i++) if (nums[i] === start + i) matched++;
    if (matched / nums.length >= 0.85) return { sequential: true, startOffset: nums[0] };
  }
  let increasing = 0;
  for (let i = 1; i < nums.length; i++) if (nums[i] > nums[i - 1]) increasing++;
  if (nums.length > 3 && increasing / (nums.length - 1) >= 0.95) {
    const maxGap = Math.max(...nums.slice(1).map((n, i) => Math.abs(n - nums[i])));
...
export function analyzeColumns(headers: string[], rows: string[][]): ColumnInsight[] {
  ...
  for (let col = 0; col < width; col++) {
    ...
    if (values.length === 0) {
      insights.push({ index: col, header, kind: "empty", confidence: 1, samples: [], filled: 0, total, unique: false, sequential: false, note: "عمود فارغ" });
      continue;
    }
    ...
    const textHits = values.filter((v) => !EMAIL_RE.test(v) && !PHONE_RE.test(v) && !PLAIN_INT_RE.test(v) && v.length > 1).length;
    const ratio = (n: number) => n / values.length;
    const { sequential } = intHits === values.length ? detectSequence(sampleRows.map((r) => (r[col] ?? "").trim()).filter(Boolean)) : { sequential: false };

    let kind: ColumnKind = "text";
    let confidence = 0.4;
    let note = "نصوص عامة";

    if (ratio(emailHits) >= 0.5) { kind = "email"; confidence = ratio(emailHits); note = "بريد إلكتروني"; }
    else if (ratio(phoneHits) >= 0.5) { kind = "phone"; confidence = ratio(phoneHits); note = "هاتف"; }
    else if (ratio(dateHits) >= 0.5) { kind = "date"; confidence = ratio(dateHits); note = "تاريخ"; }
    else if (sequential && intHits === values.length) { kind = "serial"; confidence = 0.95; note = "ترقيم تسلسلي"; }
    else if (ratio(countryHits) >= 0.4 || headerNorm.includes("دول") || headerNorm.includes("country") || headerNorm.includes("جنسي")) {
      kind = "country"; confidence = Math.max(ratio(countryHits), 0.6); note = "دولة";
    }
    else if (ratio(federationHits) >= 0.3 || headerNorm.includes("اتحاد") || headerNorm.includes("federation") || headerNorm.includes("نادي")) {
      kind = "federation"; confidence = Math.max(ratio(federationHits), 0.55); note = "اتحاد";
    }
    else if (headerNorm.includes("اسم") || headerNorm.includes("name") || headerNorm.includes("مشارك")) {
      kind = "name"; confidence = 0.85; note = "اسم";
    }
    else if (intHits === values.length && uniqueRatio >= 0.85) { kind = "id"; confidence = 0.7; note = "معرف"; }
    else if (ratio(textHits) >= 0.6 && uniqueRatio > 0.5) { kind = "name"; confidence = 0.55; note = "يبدو عمود أسماء"; }
    else if (intHits === values.length) { kind = "mixed"; confidence = 0.4; note = "أرقام"; }

    insights.push({ index: col, header, kind, confidence, samples, filled: values.length, total, unique, sequential, note });
  }
  return insights;
}
...
function mappingFromAnalysis(analysis: ColumnInsight[], rows: string[][]): ColumnMapping {
  ...
  const pick = (kind: ColumnKind) => {
    const matches = analysis.filter((c) => c.kind === kind).sort((a, b) => b.confidence - a.confidence || a.index - b.index);
  ...
  if (mapping.nameIdx === -1) {
    const candidates = analysis
      .filter((c) => c.kind === "text" || c.kind === "mixed")
      .filter((c) => c.index !== mapping.emailIdx && c.index !== mapping.phoneIdx && c.index !== mapping.countryIdx && c.index !== mapping.federationIdx && c.index !== mapping.codeIdx)
      .sort((a, b) => b.filled - a.filled || a.index - b.index);
    mapping.nameIdx = candidates[0]?.index ?? -1;
  }
  if (mapping.federationIdx === -1 && mapping.countryIdx !== -1) mapping.federationIdx = mapping.countryIdx;
  return mapping;
}

function buildWarnings(analysis: ColumnInsight[], mapping: ColumnMapping, rows: string[][]): string[] {
  const warnings: string[] = [];
  if (mapping.nameIdx === -1) warnings.push("لم يتم التعرف على عمود الأسماء تلقائياً — يرجى تحديده يدوياً.");
  if (rows.length === 0) warnings.push("الملف لا يحتوي على صفوف بيانات صالحة.");
  return warnings;
}
